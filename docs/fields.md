...
| `belongs_to`              | nil-safe link via target `label`             | values in the list (several, "(empty)"); text above `select_limit` | v2   | resolves by `identify_by`                                                            |
| `has_many` / habtm        | "a, b +n more" links                         | text over the children's `label` (columnless label: facet) | no   | "+n more" links to nested/filtered index; lists only what the ability may see (`scope_by_ability: false` opts out) |
