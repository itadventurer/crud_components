...
- **Columns the application already treats as sensitive are never printed.** A column whose name matches `config.filtered_columns` renders as `[FILTERED]` and is not filterable, sortable, searchable, editable or in the permit list. The default is the app's own `config.filter_parameters` (minus `email`/`phone`, which are data rather than secrets), so an app that curated that list gets it for free; `filtered: true/false` per attribute overrides, and `config.filtered_columns = []` switches it off. Mounting the admin on an app with a Devise `User` otherwise puts `encrypted_password` and the reset tokens on the screen. ([#60](https://github.com/itadventurer/crud_components/issues/60))
