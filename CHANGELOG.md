...
- `nested:` takes **`build:`**, and a record that isn't there yet is now built by default — so the form can create one, which is what a form for a record that may not exist yet is for. Pair it with `reject_if: :all_blank` on the model, or a submission with those fields left empty fails on the blank record's own validations. `build: false` says the opposite: no record, no block, and nothing built behind your back — the choice for a record only some parents should ever have. Before this, a parent without one got an empty bordered box with a heading and nothing in it. ([#98](https://github.com/itadventurer/crud_components/issues/98))
