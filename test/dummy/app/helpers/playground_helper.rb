...
module PlaygroundHelper
  ...
  def docs_panel(intro:, docs: {}, &)
    return if params[:bare] # ?bare=1 hides the panel — used when capturing screenshots

    # Playground prose, written in the view next to the call — never a request value.
    render 'shared/docs_panel', intro: intro.html_safe, docs: docs, body: capture(&) # rubocop:disable Rails/OutputSafety
  end
  ...
  def code_file(path, lang = :ruby, &)
    # Same: the code sample comes from the template, entity-escaped by hand.
    render 'shared/code_file', path: path, lang: lang.to_s, code: capture(&).to_s.strip.html_safe # rubocop:disable Rails/OutputSafety
  end
