...
module PlaygroundHelper
  ...
  def docs_panel(intro:, docs: {}, &)
    ...
    render 'shared/docs_panel', intro: intro.html_safe, docs: docs, body: capture(&)
  end
  ...
  def code_file(path, lang = :ruby, &)
    render 'shared/code_file', path: path, lang: lang.to_s, code: capture(&).to_s.strip.html_safe
  end
