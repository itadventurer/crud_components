...
class SemanticRenderersTest < ActiveSupport::TestCase
  # The name→renderer rule lives on StringField (folded in from the old
  # SemanticRenderer module): gated on the column name, never the value.
  test 'StringField#smart_renderer maps email/url-ish names, leaves the rest alone' do
    r = ->(name) { CrudComponents::Fields::StringField.new(name, Author).smart_renderer }
