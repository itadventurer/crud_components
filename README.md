...
| `belongs_to`              | nil-safe link via target's `label`           | select of the targets occurring in the list, valued by `identify_by` (a combobox over the target's `label` above `combobox_threshold`) | `where(assoc: Target.where(identify_by => v))`          | v2          |
...
`crud-combobox` (suggestions under a belongs_to filter with many choices) — and
depends on no JS. →
...
CrudComponents.configure { |config| … }     # css/icon maps, combobox_threshold, defaults
```
...
| [docs/performance.md](docs/performance.md) | Eager-loading, the belongs_to select→combobox threshold, pagination              |
